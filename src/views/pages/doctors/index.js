...
const fields = ['', 'name', 'email', 'speciality', 'status', 'actions'];
...
const Users = (props) => {
  ...
  const fetchUsers = async () => {
    try {
      let {ok, data} = await props.callApi(RestApi.getDoctors);
      if (ok) {
      ...
      console.log(tag, 'FETCH_Doctors_ERROR', e.message)
    }
  };
  ...
  const onClickDelete = async (id) => {
    try {
      ...
      response = await props.callApi(RestApi.deleteDoctor, id);
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      if (createMode) {
        response = await props.callApi(RestApi.createDoctor, selectedUser);
      } else {
        await idValidator.validate({id});
        response = await props.callApi(RestApi.updateDoctor, id, selectedUser);
      }
    ...
    <>
        ...
        <CCol xs="12" lg="12">
          {props.isLoading === false ?
            <CCard>
                ...
                <CDataTable
                  ...
                  scopedSlots={{
                    ...
                    'speciality':
                      (item) => (
                        <td>
                          <CBadge size={"sm"} color={"secondary"}>
                            {Constants.DOCTOR_SPECIALITY[item.speciality]}
                          </CBadge>
                        </td>
                      ),
                    'status':
              ...
              <CCardFooter>
                <CButton color="secondary" onClick={handleAdd}>Add {category}</CButton>
              </CCardFooter>
            </CCard>
            :
            <Loading/>}
      ...
      <>
                    ...
                    <CForm action="" method="post" encType="multipart/form-data" className="form-horizontal">
                      ...
                      <CFormGroup row>
                        <CCol md="6" className="text-right">
                          <CLabel htmlFor="email-input">Speciality</CLabel>
                        </CCol>
                        <CCol xs="12" md="6">
                          <h4>
                          <CBadge color={"secondary"}>{Constants.DOCTOR_SPECIALITY[selectedUser.speciality]}</CBadge>
                          </h4>
                        </CCol>
                      </CFormGroup>
                      <CFormGroup row>
                        <CCol md="6" className="text-right">
                          <CLabel htmlFor="password-input">Gender</CLabel>
                        </CCol>
                        <CCol xs="12" md="6">
                      ...
                      <CFormGroup row>
                        <CCol md="6" className="text-right">
                          <CLabel htmlFor="password-input">Address</CLabel>
                        </CCol>
                        <CCol xs="12" md="6">
                          <p
                            className="form-control-static">{`${selectedUser.street}, ${selectedUser.city}, ${selectedUser.country}`}</p>
                        </CCol>
                      </CFormGroup>
                      <CFormGroup row>
                        <CCol md="6" className="text-right">
        ...
        >
          <CModalHeader closeButton>
            <CModalTitle>
              {createMode ? "Add Doctor" : "Edit Doctor"}
            </CModalTitle>
          </CModalHeader>
          <CModalBody>
            <CRow>
                  ...
                  <CCardBody>
                    <CForm action="" method="post" encType="multipart/form-data" className="form-horizontal">
                      ...
                      <CFormGroup row>
                        ...
                        <CCol xs="12" md="9">
                          <CInput type="text" id="street-input" name="street-input" placeholder="Type a Street"
                                  autoComplete="street"
                      ...
                      <CFormGroup row>
                        ...
                        <CCol xs="12" md="9">
                          <CInput type="text" id="city-input" name="city-input" placeholder="Type a City"
                                  autoComplete="city"
                      ...
                      <CFormGroup row>
                        ...
                        <CCol xs="12" md="9">
                          <CSelect custom name="country" id="country"
                      ...
                      <CFormGroup row>
                        <CCol md="3">
                          <CLabel htmlFor="speciality">Speciality</CLabel>
                        </CCol>
                        <CCol xs="12" md="9">
                          <CSelect custom name="speciality" id="speciality"
                                   value={selectedUser.speciality}
                                   onChange={e => handleEdit('speciality', e.target.value)}
                          >
                            <option value="-1">Please select a speciality</option>
                            {
                              Object.keys(Constants.DOCTOR_SPECIALITY).map((key, index) => {
                                return (
                                  <option value={key} key={index}>{Constants.DOCTOR_SPECIALITY[key]}</option>
                                )
                              })
                            }
                          </CSelect>
